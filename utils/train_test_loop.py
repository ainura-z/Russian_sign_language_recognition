...
def train(
    model, optimizer, train_loader, device, epoch, logger, criterion, log_every=10
):
    """
    ...
    if logger:
        logger.log(
            {f"Train Accuracy": train_accuracy, f"Train loss": train_loss},
            step=epoch,
            commit=False,
        )
    if epoch % log_every == 0:
        print(
            "Epoch: {}, Train Loss: {:.4f}, Train Accuracy: {:.2f}%".format(
...
def test(
    model, test_loader, device, epoch, logger, criterion, log_every=10, wandb_prefix=""
):
    """
    ...
    with torch.no_grad():
        ...
        if logger:
            logger.log(
                {f"Test Accuracy": test_accuracy, f"Test loss": test_loss},
                step=epoch,
                commit=False,
            )
    ...
    for epoch in tqdm(range(0, num_epochs)):
        ...
        for fold in range(k_folds):
            ...
            for i, data in enumerate(train_dataloaders[fold], 0):
                # Get inputs
                inputs, targets = data[0].to(device).float(), data[1].type(
                    torch.LongTensor
                ).to(device)
                ...
                # store predicted and true values of labels
        ...
        log_dict = {
            "train_loss": np.mean(
                [results[key] for key in results.keys() if "train_loss" in key]
            ),
            "train_acc": np.mean(
                [results[key] for key in results.keys() if "train_acc" in key]
            ),
            "test_loss": np.mean(
                [results[key] for key in results.keys() if "test_loss" in key]
            ),
            "test_acc": np.mean(
                [results[key] for key in results.keys() if "test_acc" in key]
            ),
        }
        # logging cls acc
        for i in range(len(metrics)):
            log_dict[f"{i}_test_cls_acc"] = np.mean(
                [results[key] for key in results.keys() if f"{i}_test_cls_acc" in key]
            )
            log_dict[f"{i}_train_cls_acc"] = np.mean(
                [results[key] for key in results.keys() if f"{i}_train_cls_acc" in key]
            )
